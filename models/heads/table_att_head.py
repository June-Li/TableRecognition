...
class TableAttentionHead(nn.Module):
    ...
    def forward(self, inputs, targets=None):
        ...
        hidden = torch.zeros((batch_size, self.hidden_size)).to(fea.device)
        output_hiddens = []
        if self.training and targets is not None:
            structure = targets[0]
            for i in range(self.max_elem_length + 1):
                elem_onehots = self._char_to_onehot(structure[:, i], onehot_dim=self.elem_num)
                hidden, alpha = self.structure_attention_cell(hidden, fea, elem_onehots)
                outputs = hidden
            ...
            temp_elem = torch.zeros(size=[batch_size], dtype=torch.int64).to(fea.device)
            structure_probs = None
            loc_preds = None
            elem_onehots = None
            outputs = None
            alpha = None
            max_elem_length = torch.tensor(self.max_elem_length)
            i = 0
            while i < max_elem_length + 1:
                ...
                # temp_elem = structure_probs_step.argmax(axis=1)
                temp_elem = torch.argmax(structure_probs_step, dim=1)
                i += 1
...
class AttentionGRUCell(nn.Module):
    ...
    def forward(self, prev_hidden, batch_H, char_onehots):
        ...
        # context = torch.squeeze(torch.matmul(alpha, batch_H), dim=1)  # pytorch多维矩阵相乘
        context = torch.squeeze(
            torch.cat([torch.mm(alpha[i], batch_H[i]) for i, _ in enumerate(alpha)]),
            dim=1
        )  # pytorch二维模拟多维矩阵相乘
...
class AttentionLSTM(nn.Module):
    ...
    def forward(self, inputs, targets=None, batch_max_length=25):
        ...
        if targets is not None:
            ...
            targets = torch.zeros(size=[batch_size], dtype=torch.int32)
            probs = None
