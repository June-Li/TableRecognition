...
class TableSystem(object):
    ...
    def __call__(self, img, image_file):
        ...
        # my output->开始
        print('html: \n', structure_res[0])
        open('../test_out/FundScan/' + image_file.split('/')[-1].replace('.jpg', '.html').replace('.png', '.html'), 'w').write(
        ...
        print(structure_res[1])
        print(len(structure_res[0]), len(structure_res[1]))
        show_img = ori_im.copy()
        for box in structure_res[1]:
            cv2.rectangle(show_img, tuple(box[:2]), tuple(box[2:]), (0, 0, 255), 2)
        cv2.imwrite('../test_out/FundScan/' + image_file.split('/')[-1], show_img)
        # my output->结束
...
def main(args):
    ...
    for i, image_file in enumerate(image_file_list):
        ...
        if img is None:
            ...
            continue
        starttime = time.time()
        pred_html = text_sys(img, image_file)
...
if __name__ == "__main__":
    # python3 table/predict_table.py --det_model_dir=../inference_model/baidu/infer/det/ --rec_model_dir=../inference_model/baidu/infer/rec/ --table_model_dir=../inference_model/juneli/infer/table_rec/ --rec_char_dict_path=../ppocr/utils/dict/table_dict.txt --table_char_dict_path=../ppocr/utils/dict/table_structure_dict.txt --rec_char_type=EN --det_limit_side_len=736 --det_limit_type=min --output ../test_out/FundScan --image_dir=../test_data/FundScan
    args = parse_args()
    if args.use_mp:
        p_list = []
